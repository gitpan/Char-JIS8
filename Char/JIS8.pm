...
######################################################################
#
# Char::JIS8 - Source code filter to escape JIS8 script
#
# http://search.cpan.org/dist/Char-Char::JIS8/
#
...
my $function_ord;         # ord()   to ord()   or Char::JIS8::ord()
my $function_ord_;        # ord     to ord     or Char::JIS8::ord_
my $function_reverse;     # reverse to reverse or Char::JIS8::reverse
my $function_getc;        # getc    to getc    or Char::JIS8::getc
...
perl $0 JIS8_script.pl > Escaped_script.pl.e
END
    }

    print Char::JIS8::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Ejis8::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::JIS8::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Ejis8::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::JIS8::escape_script {
    ...
    # read JIS8 script
    my $fh = gensym();
    Char::Ejis8::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ejis8(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Ejis8 %s.0;\n", $Char::JIS8::VERSION); # require run-time routines version

        # use Char::JIS8 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        $function_getc    = 'getc';
        if (s/^ \s* use \s+ Char::JIS8 \s* ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::JIS8::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/JIS8.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/JIS8.pm $version, but @{[__FILE__]} is version $Char::JIS8::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ejis8.pm $version, but \\\$Char::Ejis8::VERSION is \$Char::Ejis8::VERSION" if \$Char::Ejis8::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::JIS8::VERSION) {
                    die "Script $0 required Char/JIS8.pm $version, but @{[__FILE__]} is only version $Char::JIS8::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::JIS8::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::JIS8::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::JIS8::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::JIS8::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::JIS8::re_t=%s; while($Char::JIS8::re_t =~ %s){%s local $^W=0; local $Char::JIS8::re_r=%s; %s$Char::JIS8::re_t="$`$Char::JIS8::re_r$'"; pos($Char::JIS8::re_t)=length "$`$Char::JIS8::re_r"; } return $Char::JIS8::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::JIS8::re_r=eval $Char::JIS8::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::JIS8::re_r=%s; %s"%s$Char::JIS8::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::JIS8::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::JIS8::re_r=%s; %s%s="$`$Char::JIS8::re_r$'"; pos(%s)=length "$`$Char::JIS8::re_r"; $Char::JIS8::re_n++} return %s$Char::JIS8::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::JIS8::re_r=%s; %s%s="%s$Char::JIS8::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::JIS8::re_r=%s; %s%s="%s$Char::JIS8::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::JIS8::re_r=eval $Char::JIS8::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/JIS8.pm and Char/Ejis8.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::JIS8;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::JIS8;
  use Char::JIS8 ver.sion;             --- require minimum version
  use Char::JIS8 ver.sion.0;           --- expects version (match or die)

  # "no Char::JIS8;" not supported

  or

  $ perl Char/JIS8.pm JIS8_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  JIS8_script.pl  --- script written in JIS8
...
Char::JIS8 software is "middleware" between perl interpreter and your Perl script
written in JIS8.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain JIS8, Perl3 or later
can treat JIS8 as binary data.

By "use Char::JIS8;", it automatically interpret your script as JIS8. The various
functions of perl including a regular expression can treat JIS8 now.
...
This software is a source code filter to escape Perl script encoded by JIS8
...
=item * Remains One Language Handling Raw JIS8, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/JIS8.pm, Char/Ejis8.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Ejis8.pm
works well.
...
=head1 Software Composition
   ...
   Char/JIS8.pm               --- source code filter to escape JIS8
   Char/Ejis8.pm              --- run-time routines for Char/JIS8.pm
...
You need write 'use Char::JIS8;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::JIS8;
  ---------------------------------

=head1 Calling 'Char::Ejis8::ignorecase()' (Char/JIS8.pm provides)

Char/JIS8.pm applies calling 'Char::Ejis8::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Ejis8::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Ejis8.pm provides)
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/JIS8.pm provides)

Char/JIS8.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/JIS8.pm and Char/Ejis8.pm provide)

Insert 'Char::Ejis8::' at head of function name. Char/Ejis8.pm provides your script Char::Ejis8::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Ejis8::lc        Character
  lcfirst     Char::Ejis8::lcfirst   Character
  uc          Char::Ejis8::uc        Character
  ucfirst     Char::Ejis8::ucfirst   Character
  fc          Char::Ejis8::fc        Character
  chr         Char::Ejis8::chr       Character
  glob        Char::Ejis8::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Subroutines (Char/JIS8.pm provides)

Char/JIS8.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/Char::JIS8 and change
'use utf8;' to 'use Char::JIS8;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Ejis8.pm provides these
  functions.
...
Char::JIS8 software remains one language and one interpreter.
