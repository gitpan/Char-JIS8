...
my $qq_substr  = qr{(?: JIS8::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_ord;         # ord()   to ord()   or JIS8::ord()
my $function_ord_;        # ord     to ord     or JIS8::ord_
my $function_reverse;     # reverse to reverse or JIS8::reverse
my $function_getc;        # getc    to getc    or JIS8::getc
...
END
    ...
    print JIS8::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Ejis8::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
    ...
    my $fh = gensym();
    Ejis8::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub JIS8::escape_script {
    ...
    # read JIS8 script
    my $fh = gensym();
    Ejis8::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Ejis8(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ejis8 %s.0;\n", $JIS8::VERSION); # require run-time routines version

        # use JIS8 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        $function_getc    = 'getc';
        if (s/^ \s* use \s+ JIS8 \s* ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $JIS8::VERSION) {
                    my @file = grep -e, map {qq{$_/JIS8.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects JIS8.pm $version, but @{[__FILE__]} is version $JIS8::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ejis8.pm $version, but \\\$Ejis8::VERSION is \$Ejis8::VERSION" if \$Ejis8::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $JIS8::VERSION) {
                    die "Script $0 required JIS8.pm $version, but @{[__FILE__]} is only version $JIS8::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'JIS8::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'JIS8::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'JIS8::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'JIS8::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $JIS8::re_t=%s; while($JIS8::re_t =~ %s){%s local $^W=0; local $JIS8::re_r=%s; %s$JIS8::re_t="$`$JIS8::re_r$'"; pos($JIS8::re_t)=length "$`$JIS8::re_r"; } return $JIS8::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$JIS8::re_r=eval $JIS8::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $JIS8::re_r=%s; %s"%s$JIS8::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $JIS8::re_n=0; while(%s =~ %s){%s local $^W=0; local $JIS8::re_r=%s; %s%s="$`$JIS8::re_r$'"; pos(%s)=length "$`$JIS8::re_r"; $JIS8::re_n++} return %s$JIS8::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $JIS8::re_r=%s; %s%s="%s$JIS8::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $JIS8::re_r=%s; %s%s="%s$JIS8::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$JIS8::re_r=eval $JIS8::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download JIS8.pm and Ejis8.pm and put it in your perl lib directory.

=item * You'll need to write "use JIS8;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use JIS8;
  use JIS8 ver.sion;             --- require minimum version
  use JIS8 ver.sion.0;           --- expects version (match or die)

  # "no JIS8;" not supported

  or

  $ perl JIS8.pm JIS8_script.pl > Escaped_script.pl.e
...
JIS8 software is "middleware" between perl interpreter and your Perl script
written in JIS8.
...
By "use JIS8;", it automatically interpret your script as JIS8. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. JIS8.pm, Ejis8.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Ejis8.pm
works well.
...
=head1 Software Composition
   ...
   JIS8.pm               --- source code filter to escape JIS8
   Ejis8.pm              --- run-time routines for JIS8.pm
...
You need write 'use JIS8;' in your script.
...
=head1 Calling 'Ejis8::ignorecase()' (JIS8.pm provides)

JIS8.pm applies calling 'Ejis8::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Ejis8::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Ejis8.pm provides)
...
=head1 Un-Escaping \ of \N, \p, \P and \X (JIS8.pm provides)

JIS8.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (JIS8.pm and Ejis8.pm provide)

Insert 'Ejis8::' at head of function name. Ejis8.pm provides your script Ejis8::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Ejis8::lc        Character
  lcfirst     Ejis8::lcfirst   Character
  uc          Ejis8::uc        Character
  ucfirst     Ejis8::ucfirst   Character
  fc          Ejis8::fc        Character
  chr         Ejis8::chr       Character
  glob        Ejis8::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Subroutines (JIS8.pm provides)

JIS8.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/JIS8 and change
'use utf8;' to 'use JIS8;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ejis8.pm provides these
  functions.
...
JIS8 software remains one language and one interpreter.
