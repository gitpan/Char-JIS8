...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Ejis8::index($name, '::') == -1) && (Ejis8::index($name, "'") == -1)) {
            my $pkg;
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $JIS8::re_t=%s; while($JIS8::re_t =~ %s){%s local $^W=0; local $JIS8::re_r=%s; %s$JIS8::re_t="$`$JIS8::re_r$'"; pos($JIS8::re_t)=length "$`$JIS8::re_r"; } return $JIS8::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$JIS8::re_r=CORE::eval $JIS8::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $JIS8::re_r=%s; %s"%s$JIS8::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $JIS8::re_n=0; while(%s =~ %s){%s local $^W=0; local $JIS8::re_r=%s; %s%s="$`$JIS8::re_r$'"; pos(%s)=length "$`$JIS8::re_r"; $JIS8::re_n++} return %s$JIS8::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $JIS8::re_r=%s; %s%s="%s$JIS8::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $JIS8::re_r=%s; %s%s="%s$JIS8::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$JIS8::re_r=CORE::eval $JIS8::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
